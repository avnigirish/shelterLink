  ...
  if (entries.length === 0) {
    return (
      <section aria-label="Inventory" className="mt-6">
        <h3 className="text-lg font-semibold text-text mb-3">Inventory</h3>
        <p className="text-text-subtle text-sm">No inventory listed</p>
      </section>
    );
  }
    ...
    <section aria-label="Inventory" className="mt-6">
      ...
      <table className="w-full text-sm border border-surface-border rounded-lg overflow-hidden">
        <thead className="bg-gray-50">
          <tr>
            <th className="text-left px-4 py-2 font-medium text-text">Item</th>
            <th className="text-left px-4 py-2 font-medium text-text">Quantity</th>
        ...
        <tbody className="divide-y divide-surface-border">
          {entries.map(([item]) => (
            <tr key={item}>
              <td className="px-4 py-2 text-text capitalize">{item}</td>
              <td className="px-4 py-2">
                {isAdmin ? (
                  <input
                    ...
                    className="w-24 border border-surface-border rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    aria-label={`Quantity for ${item}`}
                  />
                ) : (quantities[item] ?? 0) === 0 ? (
                  <span className="inline-block text-xs px-2 py-0.5 rounded bg-red-100 text-red-700 font-medium">
                    Out of stock
                  </span>
                ) : (
                  <span className="text-text">{quantities[item]}</span>
                )}
              </td>
              {isAdmin && (
